...
def delete_directory_if_non_empty(dir_path):
    ...
    if not os.path.exists(dir_path):
        log.info(f"目录 '{dir_path}' 不存在，无需删除。")
        return False

    # 确认路径是一个目录
    if not os.path.isdir(dir_path):
        log.info(f"'{dir_path}' 不是一个目录。")
        return False

    # 检查目录是否为空
    if not os.listdir(dir_path):
        log.info(f"目录 '{dir_path}' 为空，无需删除。")
        return False

    # 目录存在且非空，执行删除操作
    try:
        shutil.rmtree(dir_path)
        log.info(f"成功删除非空目录: '{dir_path}'")
        return True
    except Exception as e:
        log.info(f"删除目录 '{dir_path}' 时发生错误: {e}")
        return False

if __name__ == '__main__':
    # 使用示例
    # file_path = "/home/user/documents/example.txt"
    # log.info(get_filename(file_path))          # 输出: example.txt
    # log.info(get_filename(file_path, False))  # 输出: example

    # 使用示例
    directory_to_delete = "D:\\WorkSpace\\Project3-Multimodal-RAG\\output"
